...
class Header(Ach):
    ...
    def __init__(
        ...
        self.immediate_dest = self.make_right_justified(immediate_dest, 9)
    ...
    def __setattr__(self, name, value):
        if name in self.alpha_numeric_fields:
            ...
            raise AchError("%s not in alpha numeric field list" % name)
...
class FileControl(Ach):
    ...
    def __setattr__(self, name, value):
        if name in self.numeric_fields:
            ...
            raise AchError("%s not in numeric field list" % name)
...
class BatchHeader(Ach):
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.numeric_fields:
            ...
            raise AchError("%s not in numeric or alpha numeric fields list" % name)
...
class BatchControl(Ach):
    ...
    def __setattr__(self, name, value):
        if name in self.numeric_fields:
            ...
            raise AchError("%s not in numeric_fields or alpha_numeric_fields" % name)
...
class EntryDetail(Ach):
    ...
    field_lengths = {
        ...
        "ind_name": [
            15,  # CIE, MTE
            22,
        ],  # ARC, BOC, CCD, PPD, TEL, POP, POS, WEB
        "disc_data": 2,
        "id_number": 15,
        "ind_id": 22,
        "num_add_recs": 4,
    ...
    def get_row(self):
        ...
        ret_string = self.record_type_code + self.transaction_code + self.recv_dfi_id

        if len(self.recv_dfi_id) < 9:
            ret_string += self.check_digit

        ret_string += self.dfi_acnt_num + self.amount

        if self.std_ent_cls_code in ["ARC", "BOC"]:
            ret_string += self.chk_serial_num + self.ind_name + self.disc_data

        elif self.std_ent_cls_code in ["PPD", "TEL", 'CCD']:
            ret_string += self.id_number + self.ind_name + self.disc_data

        elif self.std_ent_cls_code == "CIE":
            ret_string += self.ind_name + self.ind_id + self.disc_data

        elif self.std_ent_cls_code == "CTX":
            ret_string += self.id_number + self.num_add_recs + self.recv_cmpy_name + self.reserved + self.disc_data
            ...
            ret_string += (
                self.chk_serial_num + self.terminal_city + self.terminal_state + self.ind_name + self.disc_data
            )

        elif self.std_ent_cls_code == "POS":
            ret_string += self.id_number + self.ind_name + self.card_tr_typ_code_pos

        elif self.std_ent_cls_code == "SHR":
            ret_string += self.card_exp_date + self.doc_ref_num + self.ind_card_acct_num + self.card_tr_typ_code_shr

        elif self.std_ent_cls_code == "RCK":
            ret_string += self.chk_serial_num + self.ind_name + self.disc_data

        elif self.std_ent_cls_code == "WEB":
            ret_string += self.id_number + self.ind_name + self.pmt_type_code

        ret_string += self.add_rec_ind + self.trace_num
...
class AddendaRecord(Ach):
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.alpha_numeric_fields:
            ...
            raise AchError("%s not in numeric or alpha numeric fields" % value)
    ...
    def get_row(self):
        ...
        ret_string += self.record_type_code + self.addenda_type_code
        ...
        if self.std_ent_cls_code == "MTE":
            ...
            ret_string += self.pmt_rel_info + self.add_seq_num + self.ent_det_seq_num
