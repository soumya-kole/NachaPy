...
from NachaPy.pluggable import Pluggable

from NachaPy.data_types import (
    Header,
    FileControl,
    BatchHeader,
    BatchControl,
...
class AchFile(object):
    ...
    def __init__(self, settings, pluggble_container=Pluggable):
        """
        ...
        try:
            ...
            self.header = Header(
                immediate_dest=settings["immediate_dest"],
                immediate_org=settings["immediate_org"],
                file_id_mod=pluggble_container.file_id_mod(),
                im_dest_name=settings["immediate_dest_name"],
                im_orgn_name=settings["immediate_org_name"],
            )
    ...
    def add_batch(
        ...
        for record in batch_entries:
            try:
                ...
                entry.trace_num = self.settings["trace_num"][:8] + entry.validate_numeric_field(entry_counter, 7)
